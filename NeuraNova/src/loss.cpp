...
float SoftmaxCrossEntropy::forward(const std::vector<float>& logits, const std::vector<float>& labels, 
    int output_dim, int batch_size) {
    
    if (logits.size() != labels.size()) {
        throw std::runtime_error("[ERROR] SoftmaxCrossEntropy(): logits/labels do not correspond.");
    }
