...
std::vector<float> one_hot_encode(const std::vector<uint8_t> &labels,
                                  int num_classes) {
...
FFDataSet load_and_preprocess_data_for_ff(int train_limit, int test_limit,
  ...
  int num_classes = 10;
  auto y_train_oh =
      one_hot_encode(y_train_1d, num_classes); // shape (10 * N_train)
...
FFResult build_and_train_ff_model_with_config(const FFConfig &config) {
  ...
  for (size_t i = 0; i < config.layers.size(); i++) {
    ...
    std::unique_ptr<Activation> actFn;
    if (config.layers[i].activation == "relu") {
      actFn = std::make_unique<Relu>();
    } else if (config.layers[i].activation == "sigmoid") {
      actFn = std::make_unique<Sigmoid>();
    } else {
      actFn = std::make_unique<Identity>();
    }

    std::unique_ptr<Optimizer> opt = std::make_unique<AdamOptimizer>(
        config.learning_rate, config.beta1, config.beta2, config.epsilon);

    DenseLayer layer(input_dim, out_dim, std::move(actFn), std::move(opt));
    nn.add_layer(std::move(layer));
    input_dim = out_dim;
  }

  nn.train(ds.X_train, ds.y_train, 784, ds.train_samples, config.epochs,
           ds.X_val, ds.y_val, ds.val_samples, config.batch_size);
...
void launch() {
  ...
  for (auto &cfg : configs) {
    ...
    try {
      float current_accuracy = std::stof(res.test_accuracy);
      ...
      std::cerr << "Could not convert test accuracy to float: " << e.what()
                << std::endl;
    }
  ...
  std::cout << "[FEED-FORWARD] Config with major Test Accuracy: "
            << best_config.id << std::endl;
